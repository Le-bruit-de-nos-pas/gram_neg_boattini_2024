...
df <- df %>% select(-Mortality_14d, -In_hospital_mortality)
...
# **1. Logistic Regression with LASSO (Feature Selection)**
x_train <- model.matrix(Mortality_30d ~ ., data = train_data)[, -1]  # Remove intercept
y_train <- train_data[[group_var]]
...
# **2. Random Forest Feature Importance**
rf_model <- randomForest(Mortality_30d ~ ., data = train_data, importance = TRUE)
varImpPlot(rf_model)


# **3. Model Performance using ROC-AUC**
x_test <- model.matrix(Mortality_30d ~ ., data = test_data)[, -1]
y_test <- as.numeric(test_data[[group_var]])
...
for (i in 1:nrow(df)) {
  ...
  # Train the random forest model
  rf_model <- randomForest(Mortality_30d ~ ., data = train_data, importance = TRUE, ntree = 500)
...
# **Feature Importance**
rf_final <- randomForest(Mortality_30d ~ ., data = df, importance = TRUE, ntree = 500)
importance_df <- as.data.frame(importance(rf_final))
importance_df <- importance_df[order(importance_df$MeanDecreaseGini, decreasing = TRUE), ]
